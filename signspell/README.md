...
Standing over the circle is a demon made of actual smoke. `js/visual/fluid.js` is a Navier-Stokes solver — Jos Stam's Stable Fluids (semi-Lagrangian advection, Gauss-Seidel pressure projection) with Fedkiw's vorticity confinement and thermal buoyancy, the same method the WebGL fluid demos run, on the CPU over a 64×72 grid at about 1.6 ms a frame. The figure is never drawn: it is continuously re-injected along a skeleton of bones while the solver tears it apart, and what you see is the argument between the two. Every bone owns a frequency band, so the body is the analyser — the torso swells on the low end, the arms follow the mids, the horns flare on the top. Bass drives buoyancy, the top end drives vorticity, and hits shove the chest outward. The music moves the physics, not a filter over a picture.
